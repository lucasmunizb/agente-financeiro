...
use App\Domain\Recorrencia\ConverterLancamentoEmRecorrencia;
...
class LancamentoFormController extends Controller
{
    ...
    public function update(
        ...
        ConverterLancamentoEmRecorrencia $converter,
    ): JsonResponse {
        $tx = $this->transacaoDoUsuario($request, $transaction);

        // Switch ligado num lançamento comum ⇒ CONVERSÃO (spec 12, D5): o lançamento é
        // substituído pela recorrência + a ocorrência do mês, em vez de coexistir com ela —
        // era essa coexistência que duplicava a conta no extrato. Não há mais o caminho
        // "lançamento já recorrente": recorrência não vive em `transactions` (D4).
        try {
            if ($dados = $request->dadosRecorrencia()) {
                $converter->converter($tx, $dados, CarbonImmutable::now(RelativeDate::TIMEZONE));
