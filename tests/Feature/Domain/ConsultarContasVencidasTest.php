...
it('nunca marca uma parcela vencida como recorrente: recorrência não vive em transactions (spec 12)', function () {
    $user = User::factory()->create();

    contaVencida($user, 5000, '2026-06-20', 'Netflix');
    contaVencida($user, 3000, '2026-06-19', 'Padaria');

    $contas = collect(app(ConsultarContasVencidas::class)->para($user->id, hojeVenc())->contas)
        ->keyBy('descricao');

    // A conta fixa vencida entra pelo quadro do ResumoDoMes, via ocorrência.
