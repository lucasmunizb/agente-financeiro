...
@php
    ...
    // Sem cartão cadastrado, começa em "Pix" (à vista) — crédito exige cartão e só
    // frustraria. Com cartão, mantém o padrão do design (Crédito).
    $formaInicial = $cartoes->isEmpty() ? 'pix' : 'credito';

    // Ícone da categoria: mapeia o `icone` guardado (livre) para o conjunto SVG
    // inline; cai em 'tag' quando não houver correspondência (sem CDN — regra 6).
...
{{-- Modal "Registrar gasto" (spec §7.7b) — fiel à tela do Stitch "Registrar gasto
     — Crédito". Ligado ao backend real: os cartões/categorias vêm do usuário e o
     cálculo (parcelas, vencimento, valor por parcela) é do domínio determinístico
     (regra 4 — a tela nunca calcula). Fluxo em dois passos (regra 7): o formulário
     manda para a PRÉVIA (calculada pelo backend); só após a confirmação é que grava.
     Ícones SVG inline (regra 6/LGPD). --}}
<div id="modal-registrar-gasto"
    class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop"
    data-rg-autoopen="{{ $open ? '1' : '0' }}"
    data-rg-initial="{{ $initial ?? '' }}"
    data-previa-url="{{ route('gastos.previa') }}"
    data-store-url="{{ route('gastos.store') }}"
    ...
    <div class="modal-card-enter notebook-card flex max-h-[92vh] w-full max-w-[640px] flex-col overflow-hidden rounded-card"
        ...
        {{-- ============================ PAINEL 1: FORM ============================ --}}
        <form id="rg-form" data-rg-panel="form" novalidate class="flex min-h-0 flex-1 flex-col">
            @csrf
            <input type="hidden" name="forma" value="{{ $formaInicial }}" data-rg-forma-input>
            <input type="hidden" name="categoria_id" value="" data-rg-categoria-input>
            ...
            <div class="min-h-0 flex-1 space-y-6 overflow-y-auto px-gutter py-6">
                ...
                <div data-rg-group="credito" class="space-y-6" @if ($formaInicial !== 'credito') hidden @endif>
                    @if ($cartoes->isEmpty())
                        {{-- Sem cartão cadastrado: crédito não pode prosseguir. --}}
                    ...
                    @else
                            ...
                            <div class="relative">
                                <select id="rg-card" name="card_id"
                                    ...
                                    @foreach ($cartoes as $card)
                                        <option value="{{ $card->id }}">{{ $card->descricao }} •••• {{ $card->final_4 }} — fecha dia {{ $card->dia_fechamento }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <input id="rg-parcelas" name="parcelas" type="number" value="1" min="1" max="24"
                ...
                <div data-rg-group="avista" class="space-y-6" @if ($formaInicial === 'credito') hidden @endif>
                    <div class="flex flex-col gap-2">
                        <label for="rg-vencimento" class="font-body-sm text-body-sm text-on-surface-variant">Data de vencimento *</label>
                        <input id="rg-vencimento" name="vencimento" type="date"
                    ...
                    {{-- Recorrência: no modelo, porém o backend é pós-MVP (spec §7.7b).
                         Mantido no design; ainda não é persistido. --}}
                    <div class="space-y-4">
                    ...
                    @else
                        <div class="flex flex-wrap gap-2">
                            @foreach ($categorias as $cat)
                                <button type="button" data-rg-categoria="{{ $cat->id }}" aria-pressed="false"
                                    class="flex items-center gap-2 rounded-full border border-linha bg-surface-container px-4 py-2 font-body-sm text-body-sm font-medium text-on-surface-variant transition-all hover:bg-surface-container-high active:scale-95">
                                    @if ($cat->cor)
            ...
            <footer class="flex flex-col gap-3 border-t border-linha bg-surface-container-low p-gutter md:flex-row-reverse">
                <button type="submit" data-rg-review
                    ...
                    <span data-rg-review-label>Revisar e confirmar</span>
                </button>
        ...
        <div data-rg-panel="confirm" hidden class="flex min-h-0 flex-1 flex-col">
            <div class="min-h-0 flex-1 space-y-5 overflow-y-auto px-gutter py-6">
                <div>
                    <h3 class="font-headline-md text-headline-md text-on-surface">Confirme o gasto</h3>
            ...
            <footer class="flex flex-col gap-3 border-t border-linha bg-surface-container-low p-gutter md:flex-row-reverse">
                <button type="button" data-rg-store
                    ...
                    <span data-rg-store-label>Confirmar e gravar</span>
                </button>
